...
import { activeRefs } from "../lib/roster";
...
export function FieldGame() {
  ...
  const move = useCallback(
    (dx: number, dy: number) => {
      ...
      sim.current = next;
      const seen = seenIdOnTransition(prev, next);
      if (seen) api.markSeen(seen);
  ...
  async function onPasteIdentify(e: FormEvent) {
    ...
    if (shape === "index") {
      ...
      play("catch", mute);
      sim.current = { ...sim.current, mode: "walk", toast: LAB_INDEX_TOAST };
      setPasteText("");
      setPasteResult(null);
      overlay();
      return;
    ...
    if (shape === "dump") {
      play("warn", mute);
      sim.current = { ...sim.current, mode: "walk", toast: LAB_DUMP_LECTURE };
      setPasteText("");
      setPasteResult(null);
      overlay();
      return;
    ...
    setPasteBusy(true);
    try {
      const r = await identifyInput(raw, { active: activeRefs(api.save), activeCount: api.stats.active });
      api.markSeen(r.id);
      setPasteResult(r);
      play("identify", mute);
      overlay();
    ...
    s.mode === "intro"
      ...
      : s.mode === "help"
        ...
        : s.mode === "talk"
          ...
          : s.mode === "paste"
            ? pasteResult
              ? (pasteResult.name ?? "Wild paste")
              : "Lab paste"
            : (card?.name ?? "Wild");
  const overlayFocusKey = `${s.mode}:${s.mode === "paste" && pasteResult ? "card" : s.encounterId ?? ""}`;
  ...
  useEffect(() => {
    ...
    target?.focus();
  }, [overlayOn, overlayFocusKey]);

  return (
    <section className="field-shell" aria-label="Field">
      <div className="field-canvas-wrap" ref={wrapRef}>
        <canvas ref={canvasRef} role="img" tabIndex={0} aria-label={fieldAriaLabel(s)} />
        {overlayOn ? null : (
        ...
        {overlayOn ? (
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="field-overlay-title"
          >
            <h2 id="field-overlay-title" className="sr-only">
              {overlayTitle}
            </h2>
            ...
            {s.mode === "paste" && !pasteResult ? (
              <form className="stamp-card" onSubmit={(ev) => void onPasteIdentify(ev)}>
                ...
                <p className="lede">Wish or x.ai/bot URL. Dumps belong on Desk. We never click Add.</p>
                <textarea
                  value={pasteText}
        ...
        <div className="field-pad" aria-label="Tap controls">
          ...
          <p className="field-pad-hint dim">WASD walk. Z talk. 1 2 3 stamp. S skip. Lab A pastes.</p>
