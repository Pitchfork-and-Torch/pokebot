...
import { TILE_PX, VIEW_H, VIEW_W, drawFrame } from "../lib/fieldDraw";
...
export function FieldGame() {
  ...
  const paint = useCallback(() => {
    ...
    drawFrame(ctx, sim.current, sim.current.frame, reduce, (id) => plateFor(id)?.types[0] ?? "ops");
  }, []);
  ...
  return (
    <section className="field-shell" aria-label="Field">
      <div className="field-canvas-wrap">
        <canvas ref={canvasRef} width={VIEW_W * TILE_PX} height={VIEW_H * TILE_PX} role="img" aria-label="Field map. WASD to walk." />
        {overlayOn ? null : (
