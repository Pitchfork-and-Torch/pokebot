...
export function drawFrame(
  ...
  const camX = Math.max(0, Math.min(state.x - Math.floor(VIEW_W / 2), MAP_W - VIEW_W));
  const camY = Math.max(0, Math.min(state.y - Math.floor(VIEW_H / 2), MAP_H - VIEW_H));
  ctx.imageSmoothingEnabled = false;
  fill(ctx, "#0b0f0c", 0, 0, VIEW_W * TILE_PX, VIEW_H * TILE_PX);
  ...
  for (const actor of state.actors) {
    ...
    if (ax < -1 || ay < -1 || ax > VIEW_W || ay > VIEW_H) continue;
