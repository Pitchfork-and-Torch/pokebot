...
import { TYPE_META, specimenToIdentify } from "../data/types";
...
export function identifyPlate(spec: Specimen, save: SaveFile): IdentifyResult {
  ...
  if (sameType) {
    const held = TYPE_META[spec.types[0] ?? "ops"].label;
    return {
      ...base,
      verdict: "WATCH",
      why: [`${sameType.name} already holds ${held}. Stamp Watch.`],
      slot_advice: "box",
    };
  }
  return base;
...
export function seenIdOnTransition(prev: FieldState, next: FieldState): string | null {
  if (next.mode !== "encounter" || prev.mode === "encounter") return null;
  return next.encounterId;
}

export function fieldAriaLabel(state: FieldState): string {
  if (state.mode === "intro") return "Title screen. Press A to walk the field.";
  if (state.mode === "help") return "Field help. Press B to close.";
  if (state.mode === "talk") return state.talk ?? "Professor talk.";
  if (state.mode === "paste") return "Lab paste. Type a wish or share URL, then Identify.";
  if (state.mode === "encounter") {
    const spec = state.encounterId ? plateFor(state.encounterId) : undefined;
    if (!spec) return "Wild encounter. 1 Keep, 2 Watch, 3 Skip.";
    return `Wild ${spec.name}. ${spec.title}. 1 Keep, 2 Watch, 3 Skip.`;
  }
  return "Field map. WASD to walk. Z talks. Lab A pastes.";
}

export const LAB_DUMP_LECTURE = "A dump is many jobs. Desk paste queues hearings. Lab is one wish or one URL.";

export const LAB_INDEX_TOAST = "INDEX.md loaded. File of record replaced the save.";
...
export function cancel(state: FieldState): FieldState {
  ...
  if (state.mode === "encounter") {
    const toast = state.encounterUid ? "Left the wild standing." : "It slipped back into the grass.";
    return { ...state, mode: "walk", encounterId: null, encounterUid: null, toast };
  }
...
export function stampKey(key: string): "catch" | "watch" | "skip" | null {
  ...
  if (key === "3" || key === "s" || key === "S") return "skip";
  return null;
}
