...
export function plateVerdict(s: Specimen): Verdict {
  if (s.origin === "duplicate" || s.origin === "stub") return "WATCH";
  if (s.risk === "high") return "SKIP";
  if (s.never_list.length === 0) return "WATCH";
  return "CATCH";
}

export function plateSlotAdvice(s: Specimen): SlotAdvice {
  const verdict = plateVerdict(s);
  if (verdict === "SKIP") return "do-not-create";
  if (verdict === "WATCH") return "box";
  return "pin";
}
