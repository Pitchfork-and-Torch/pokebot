...
import { TYPE_IDS, TYPE_META, specimenToIdentify, type TypeId } from "../data/types";
...
export function Dex() {
  ...
  const canCatchSeed = Boolean(spec && book === "field" && !owned && spec.risk !== "high" && spec.origin !== "duplicate");
      ...
      {selected && spec && (
        <>
          ...
          <div className="row">
            ...
            {spec.origin === "duplicate" || spec.risk === "high" ? <span className="dim">Teaching specimen. Leave it off the six.</span> : null}
          </div>
        </>
      )}
      {selected && !spec && (
